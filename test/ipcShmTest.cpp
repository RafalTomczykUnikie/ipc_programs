...
class IpcShmTest : public ::testing::Test
{
protected:
    ...
    static constexpr auto SHM_BUF_SIZE = (32u * (1<<20u));

};

void IpcShmTest::SetUp(void)
{
    ...
    m_file_sender = new ShmFileSender(m_comand_sender, "shm_buffer", "shm_sem_prod", "shm_sem_cons", SHM_BUF_SIZE);
    m_file_receiver = new ShmFileReceiver(m_comand_receiver, "shm_buffer", "shm_sem_prod", "shm_sem_cons", SHM_BUF_SIZE);    
}

void IpcShmTest::TearDown(void)
{
    delete m_server;
    delete m_client;
    delete m_comand_receiver;
    delete m_comand_sender;
    delete dynamic_cast<ShmFileReceiver *>(m_file_receiver);
    delete dynamic_cast<ShmFileSender *>(m_file_sender);
}

TEST_F(IpcShmTest, TestConstructor)
{
    auto *snd = dynamic_cast<ShmFileSender *>(m_file_sender);
    auto *rcv = dynamic_cast<ShmFileReceiver *>(m_file_receiver);

    EXPECT_NE(snd->m_shm_file_descriptor, 0);
    EXPECT_NE(rcv->m_shm_file_descriptor, 0);
    
    EXPECT_EQ(snd->m_semaphore_name_prod, "shm_sem_prod");
    EXPECT_EQ(rcv->m_semaphore_name_prod, "shm_sem_prod");
    
    EXPECT_EQ(snd->m_semaphore_name_cons, "shm_sem_cons");
    EXPECT_EQ(rcv->m_semaphore_name_cons, "shm_sem_cons");

    EXPECT_EQ(snd->m_shm_name, "shm_buffer");
    EXPECT_EQ(rcv->m_shm_name, "shm_buffer");

    EXPECT_NE(snd->m_command_sender, nullptr);
    EXPECT_NE(rcv->m_command_receiver, nullptr);

    EXPECT_NE(snd->m_memptr, nullptr);
    EXPECT_NE(rcv->m_memptr, nullptr);

    EXPECT_NE(snd->m_semaphore_handle_prod, nullptr);
    EXPECT_NE(rcv->m_semaphore_handle_prod, nullptr);
    
    EXPECT_NE(snd->m_semaphore_handle_cons, nullptr);
    EXPECT_NE(rcv->m_semaphore_handle_cons, nullptr);
    
    EXPECT_EQ(snd->m_shm_buff_size, SHM_BUF_SIZE);
    EXPECT_EQ(snd->m_shm_buff_size, SHM_BUF_SIZE);
}

TEST_F(IpcShmTest, TestFileSendingReceivingSmallFile)
{
...
TEST_F(IpcShmTest, TestFileSendingReceivingLargeFile)
{
...
TEST_F(IpcShmTest, TestWrongFileAgreement)
{
