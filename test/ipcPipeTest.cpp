...
void IpcPipeTest::SetUp(void)
{
    ...
    m_file_receiver = new PipeFileReceiver(m_comand_receiver, "/tmp/test_pipe");
    m_file_sender = new PipeFileSender(m_comand_sender, "/tmp/test_pipe");

}

void IpcPipeTest::TearDown(void)
{
    delete m_server;
    delete m_client;
    delete m_comand_receiver;
    delete m_comand_sender;
    delete dynamic_cast<PipeFileReceiver *>(m_file_receiver);
    delete dynamic_cast<PipeFileSender *>(m_file_sender);
}

TEST_F(IpcPipeTest, TestConstructor)
{
    auto *snd = dynamic_cast<PipeFileSender *>(m_file_sender);
    auto *rcv = dynamic_cast<PipeFileReceiver *>(m_file_receiver);


    EXPECT_NE(snd->m_pipe_receiver_fd, 0);
    EXPECT_NE(rcv->m_pipe_receiver_fd, 0);
    EXPECT_NE(snd->m_command_sender, nullptr);
    EXPECT_NE(rcv->m_command_receiver, nullptr);    
}

TEST_F(IpcPipeTest, TestFileSendingReceivingSmallFile)
{
...
TEST_F(IpcPipeTest, TestFileSendingReceivingLargeFile)
{
    constexpr auto data_size = (1u << 30);
    constexpr auto vector_data_size = (1u << 20);
...
TEST_F(IpcPipeTest, TestWrongFileAgreement)
{
    auto temp_file_sender = new MessageFileSender(m_comand_sender, "/test_message");
    ...
    auto sender = [&](){
        auto err_snd = temp_file_sender->connectionAgrrement("tmp_file_in", "txt", data_size);
        if(!err_snd)
            temp_file_sender->sendFile(in_file_name.c_str());
    };

    auto receiver = [&](){
        err_rcv= m_file_receiver->connectionAgrrement();
        if(!err_rcv)
            m_file_receiver->receiveFile(out_file_name.c_str());
    };
