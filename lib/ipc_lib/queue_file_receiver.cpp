...
QueueFileReceiver::QueueFileReceiver(IpcCommandReceiver *command_receiver, std::string queue_name) : 
...
QueueFileReceiver::~QueueFileReceiver()
{
    mq_close(m_queue_file_descrptors);
}

QueueFileReceiver::file_rx_agreement_t QueueFileReceiver::connectionAgrrement(void)
{
    ...
    res = RxTx(IpcCommand::ipc_command_tx_t::IPC_SEND_OVER_MESSAGE_QUEUE, 
         IpcCommand::ipc_command_rx_t::IPC_SEND_OVER_MESSAGE_QUEUE_CONFIRMED,
         IpcCommand::ipc_command_rx_t::IPC_DIFFERENT_IPC_METHOD);

    if(res)
        return file_rx_agreement_t::FILE_AGR_ERROR;

    LOG(INFO) << "File will be send over message queue - confirmation send back";
...
int QueueFileReceiver::RxTx(IpcCommand::ipc_command_tx_t tx, IpcCommand::ipc_command_rx_t rx_ok, IpcCommand::ipc_command_rx_t rx_nok)
{
...
QueueFileReceiver::file_rx_err_t QueueFileReceiver::receiveFile(const char *output_path)
{
