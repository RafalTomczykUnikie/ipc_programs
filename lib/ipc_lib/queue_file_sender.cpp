...
QueueFileSender::QueueFileSender(IpcCommandSender *command_sender, std::string queue_name) :
...
QueueFileSender::~QueueFileSender()
{
    mq_close(m_queue_file_descrptors);
}

QueueFileSender::file_tx_agreement_t QueueFileSender::connectionAgrrement(std::string file_name, std::string file_extension, size_t file_size)
{
    ...
    command = IpcCommandFactory::getCommand(IpcCommand::ipc_command_tx_t::IPC_SEND_OVER_MESSAGE_QUEUE);
    ...
    if(c_rslt != IpcCommandSender::command_send_error_t::COMMAND_SENT_OK ||
       r_rslt != IpcCommandSender::command_response_error_t::RESPONSE_OK ||
        response.response != IpcCommand::ipc_command_rx_t::IPC_SEND_OVER_MESSAGE_QUEUE_CONFIRMED)
    {
        retVal = FILE_AGREMENT_ERROR;
        return retVal;
    }
...
QueueFileSender::file_tx_err_t QueueFileSender::sendFile(const char * file_path)
{
